...
class Pick_Place:
    ...
    def generate_grasps(self, pose, width):
        ...
        for angle in numpy.arange(0.0, numpy.deg2rad(360.0), numpy.deg2rad(90.0)):
            ...
            grasp.pre_grasp_approach.desired_distance = (
                self.approach_retreat_desired_dist
            )
    ...
    def pickup(self, group, target, width):
        ...
        if state != GoalStatus.SUCCEEDED:
            rospy.logerr(
                "Pick up goal failed!: %s" % self.pickup_action.get_goal_status_text()
            )
            return None
        ...
        if err != MoveItErrorCodes.SUCCESS:
            rospy.logwarn(
                "Group %s cannot pick up target %s!: %s"
    ...
    def place(self, group, target, place):
        ...
        if state != GoalStatus.SUCCEEDED:
            rospy.logerr(
                "Pick up goal failed!: %s" % self.place_action.get_goal_status_text()
            )
            return None
