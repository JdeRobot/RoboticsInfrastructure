...
imageMsg2Image(const sensor_msgs::msg::Image::ConstSharedPtr &img) {
  ...
  cv::Mat cv_image;
  if (img->encoding.length() >= 2 &&
      img->encoding.substr(img->encoding.length() - 2) == "C1") {
...
CameraNode::CameraNode(const std::string &topic, const std::string &node_name)
  ...
  sub_ = this->create_subscription<sensor_msgs::msg::Image>(
      topic, 10,
      std::bind(&CameraNode::listener_callback, this, std::placeholders::_1));
}

void CameraNode::listener_callback(
