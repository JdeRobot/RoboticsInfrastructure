...
- Add the scene file inside the Scenes directory using a clear and simple name.
- You will also need to create a launcher for each scene and add it to the database.
- To test the new scene you will have to create a new RADI (or BTDI in BT Studio) with the Robotics Infrastructure flag (-i) set to your branch name.
- The scene name defined inside the .world file must be "default":
  ```xml
  <?xml version="1.0" ?>
  <sdf version="1.10">
    <world name="default">
...
- Add the python ros launcher inside the Launchers directory using a clear and simple name (for example the same as the scene or the exercise).
