...
namespace custom_plugins
{
                                      ...
                                      gz::sim::EventManager &)
  {
    ...
    use_pose_source_ = (_sdf->Get<std::string>("odom_source", "wheels").first == "pose");
                                       ...
                                       const gz::sim::EntityComponentManager &_ecm)
  {
    ...
    // Per-tick body-frame increments: ds forward, dn lateral (left +), dtheta yaw.
    // dn stays 0 for non-holonomic (differential) robots.
    ...
    if (use_pose_source_)
    {
      ...
      if (!initialized_)
      {
        ...
        noisy_x_ = x; noisy_y_ = y; noisy_yaw_ = yaw;
        last_pose_x_ = x; last_pose_y_ = y; last_pose_yaw_ = yaw;
        last_update_time_ = _info.simTime;
        initialized_      = true;
        return;
      }

      dt = std::chrono::duration<double>(_info.simTime - last_update_time_).count();
      if (dt <= 0.0) return;
      last_update_time_ = _info.simTime;
    ...
    const double sigma_trans = alpha3_ * std::sqrt(trans)            + alpha4_ * std::sqrt(std::abs(dtheta));
    const double sigma_rot   = alpha1_ * std::sqrt(std::abs(dtheta)) + alpha2_ * std::sqrt(trans);

    const double ds_noisy     = ds     + gz::math::Rand::DblNormal(0.0, sigma_trans);
    const double dn_noisy     = holonomic_ ? dn + gz::math::Rand::DblNormal(0.0, sigma_trans) : 0.0;
    ...
    noisy_x_   += ds_noisy * std::cos(heading_mid) - dn_noisy * std::sin(heading_mid);
    noisy_y_   += ds_noisy * std::sin(heading_mid) + dn_noisy * std::cos(heading_mid);
