...
//   <gripper_link>     link the payload is attached to      (default:
//   base_link) <attach_distance>  max distance to grab a payload, in m
//   (default: 0.15) <magnet_topic>     std_msgs/Bool, energize/de-energize
//   (default: /<model>/gripper/magnet) <state_topic>      std_msgs/Bool,
//   currently carrying?   (default: /<model>/gripper/attached)
//   <graspable_topic>  std_msgs/String, CSV grabbable names (default:
//   /<model>/gripper/graspable)
...
class DroneGripper : public System,
...
public:
  ...
  void Configure(const Entity &_entity,
    ...
    this->gripperLinkName =
        _sdf->Get<std::string>("gripper_link", "base_link").first;
    this->attachDistance = _sdf->Get<double>("attach_distance", 0.15).first;

    const std::string ns = "/" + modelName + "/gripper";
    const std::string magnetTopic =
        _sdf->Get<std::string>("magnet_topic", ns + "/magnet").first;
    const std::string stateTopic =
        _sdf->Get<std::string>("state_topic", ns + "/attached").first;
    const std::string graspableTopic =
        _sdf->Get<std::string>("graspable_topic", ns + "/graspable").first;
    ...
    this->magnetSub = this->node->create_subscription<std_msgs::msg::Bool>(
        magnetTopic, 10, [this](const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    this->statePub =
        this->node->create_publisher<std_msgs::msg::Bool>(stateTopic, 10);
  ...
  void PreUpdate(const UpdateInfo &_info,
    ...
    // keeps the DetachableJoint from outliving the drone links, which would
    // wedge the physics server and stop the drone re-spawning. Runs even while
    // paused, because reset happens with the world paused.
    if (this->activeJoint != kNullEntity && this->GripperGoneOrRemoving(_ecm)) {
      this->HandleResetDetach(_ecm);
    }

    if (_info.paused) {
      // A reset pauses the world before removing/resetting the drone. Detach
      // the box (so it is not jointed to the drone when it is removed) AND
      // de-energize the magnet. De-energizing is key: otherwise TryAttach would
      // auto-re-attach on unpause, leaving the gripper stuck "carrying" a
      // stale/reset box so it never grabs again. The exercise re-energizes the
      // magnet when it wants to grab, so a fresh run works normally. If
      // GripperGoneOrRemoving already handled it above this cycle, activeJoint
  ...
  void Reset(const UpdateInfo &, EntityComponentManager &_ecm) override {
    if (this->activeJoint != kNullEntity) {
      _ecm.RequestRemoveEntity(this->activeJoint);
      this->activeJoint = kNullEntity;
    }
...
private:
  ...
  // True if our own drone model no longer exists, or is marked for removal in
  // the current update cycle (EachRemoved reports entities erased at cycle
  // end).
...
GZ_ADD_PLUGIN(drone_gripper::DroneGripper, gz::sim::System,
