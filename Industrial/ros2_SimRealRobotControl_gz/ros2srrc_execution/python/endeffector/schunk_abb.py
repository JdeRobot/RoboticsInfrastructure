...
class SchunkGRIPPER(Node):
    ...
    def __init__(self):
        ...
        print(
            "[CLIENT - schunk_abb.py]: Initialising ABB-RWS I/O ROS 2 Service Client."
        )

        self.cli = self.create_client(SetIOSignal, "/rws_client/set_io_signal")
        while not self.cli.wait_for_service(timeout_sec=1.0):
            print(
                "[CLIENT - schunk_abb.py]: Waiting for ABB-RWS I/O Service Server to be available..."
            )
    ...
    def send_request(self, signal, value):
        ...
        print(
            "[CLIENT - schunk_abb.py]: ROS 2 Service successfully executed. Signal -> "
            + signal
            + ", Value -> "
            + value
        )
    ...
    def CLOSE(self):
        ...
        RES["Message"] = (
            "GripperClose signal successfully sent to ABB Robot Controller."
        )
        RES["Success"] = True
        return RES
