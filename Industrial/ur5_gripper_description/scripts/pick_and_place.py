...
class PickAndPlaceNode(Node):
    def __init__(self):
        ...
        self.add_collision_object(
            ...
            pose=Pose(
                position=Point(x=0.6, y=0.0, z=0.05),
        ...
        self.add_collision_object(
            ...
            pose=Pose(
                position=Point(x=-0.6, y=0.0, z=-0.17),
                orientation=Quaternion(w=0.707, x=0.0, y=0.0, z=0.707),
            ),
            frame_id="base_link",
        )
        ...
        for name, x, y in bins:
            self.add_collision_object(
                ...
                pose=Pose(
                    position=Point(x=x, y=y, z=bin_z),
        ...
        self.tasks = [
            {
                ...
                "id": "red_box",
                "width": 0.055,
                "pick": [0.6, -0.3, 0.14 + gripper_offset],
                "drop": [-0.4, 0.15, 0.0],
            },
    ...
    def execute_routine_thread(self):
        ...
        self.get_logger().info(
            "Objects are at world Z=1.04-1.07 (base_link Z=0.14-0.17)"
        )
        ...
        for task in self.tasks:
            ...
            self.get_logger().info(
                f"Step 1: Moving to pre-pick position above {task['name']}"
            )
            success = self.move_arm_to_pose(pre_pick_pose, self.down_orientation)
            if not success:
                self.get_logger().error(
                    f"Failed to move to pre-pick for {task['name']}"
                )
                continue
            ...
            if not success:
                self.get_logger().error(
                    f"Failed to reach pick position for {task['name']}"
                )
                continue
            ...
            self.get_logger().info(
                f"Step 3: Closing gripper to {target_pos:.3f} rad for object width {target_width}m"
            )
            ...
            self.get_logger().info(
                f"Step 4.5: Moving to safe home position with object"
            )
            success = self.move_arm_to_pose(home_pose, self.down_orientation)
            if not success:
            ...
            if not success:
                self.get_logger().error(
                    f"Failed to move to pre-drop location for {task['name']}"
                )
                continue
            ...
            if self.move_cartesian(drop_pose):
                ...
                self.get_logger().warn(
                    "Could not reach low drop height, dropping from pre-drop height (Fallback)"
                )
    ...
    def move_cartesian(self, target_pose, steps=10):
        ...
        req.waypoints = [
            Pose(
                position=Point(x=target_pose[0], y=target_pose[1], z=target_pose[2]),
        ...
        if not response or response.error_code.val != 1:
            self.get_logger().error(
                f"  ✗ Cartesian path computation failed (Error: {response.error_code.val if response else 'None'})"
            )
            return False

        if response.fraction < 0.9:
            self.get_logger().error(
                f"  ✗ Cartesian path incomplete (Fraction: {response.fraction})"
            )
            return False

        # Execute Trajectory
        self.get_logger().info(
            f"  → Path computed (Fraction: {response.fraction:.2f}). Executing..."
        )
        ...
        if result.result.error_code.val == 1:
            ...
            self.get_logger().error(
                f"  ✗ Cartesian execution failed (Error: {result.result.error_code.val})"
            )
            return False
    ...
    def move_arm_to_pose(self, position, orientation):
        ...
        goal_msg.request.max_velocity_scaling_factor = (
            0.2  # Increased for better reachability
        )
        ...
        oc_goal.absolute_x_axis_tolerance = (
            0.5  # Relaxed to prevent self-collisions (was 0.2)
        )
        ...
        goal_msg.request.goal_constraints.append(
            Constraints(position_constraints=[pc], orientation_constraints=[oc_goal])
        )
    ...
    def control_gripper(self, position):
        ...
        if not goal_handle:
            self.get_logger().error(
                "  ✗ Gripper goal not accepted (timeout or failure)"
            )
            return
        ...
        if result.result.error_code == 0:  # SUCCESS
            ...
            self.get_logger().warn(
                f"  ⚠ Gripper action finished with error code: {result.result.error_code}"
            )
