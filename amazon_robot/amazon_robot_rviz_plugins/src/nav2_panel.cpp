...
#include "amazon_robot_rviz_plugins/nav2_panel.hpp"
...
#include "amazon_robot_rviz_plugins/goal_common.hpp"
  ...
  client_loc_("lifecycle_manager_localization")
{
  ...
  waypoint_follower_action_client_ =
    rclcpp_action::create_client<amazon_robot_msg::action::FollowTargets>(
    client_node_,
    "FollowTargets");

  // robot_controller_action_client_ =
  // rclcpp_action::create_client<amazon_robot_msg::action::FollowTargets>(
  // client_node_,
  // "FollowTarget");
  ...
  waypoint_follower_goal_ = amazon_robot_msg::action::FollowTargets::Goal();
...
Nav2Panel::startWaypointFollowing(std::vector<geometry_msgs::msg::PoseStamped> poses)
{
  ...
  if (!is_action_server_ready) {
    RCLCPP_ERROR(
      client_node_->get_logger(), "FollowTargets action server is not available."
  ...
  auto send_goal_options =
    rclcpp_action::Client<amazon_robot_msg::action::FollowTargets>::SendGoalOptions();
...
PLUGINLIB_EXPORT_CLASS(amazon_robot_rviz_plugins::Nav2Panel, rviz_common::Panel)
