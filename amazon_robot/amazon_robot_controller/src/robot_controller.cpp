...
namespace amazon_robot_controller {
    ...
    RobotController::on_configure(const rclcpp_lifecycle::State & /*state*/) {
        ...
        blackboard_->set<bool>("joint_state", false);  // NOLINT
...
//                else {
//                    if (goal->load.size() == 0) {
//                        RCLCPP_INFO(get_logger(), "Load effort not provided. Defaulting to unload");
//                        std::vector <int32_t> load_queue {0};
//                        blackboard_->set("load_queue", load_queue);
//                    }
    ...
    RobotController::initializeBlackboard(std::shared_ptr<const Action::Goal> goal) {
        ...
        blackboard_->set("current_load_queue_idx", 0);
        blackboard_->set("load_queue", goal->load);
